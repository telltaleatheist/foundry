# boxes encoder — replay verification

`src/boxes/encoder.ts` is a MOVE of BookForgeApp
`src/app/features/pdf-picker/services/rubric-encoder.ts` (MIGRATION §1). Symbols
were renamed rubric→boxes; nothing about what the module emits or accepts was
...
| `RUBRIC_STOP` vs `BOXES_STOP` | same string value |
| `rubricCategories(v)` vs `boxesCategories(v)` | v0..v7, including the out-of-range ends |
| `rubricVersionFor()` vs `boxesVersionFor()` | 12 ids, including `foundry-boxes-v1-4b` and an id with no version |

## Result — 2026-08-01

Command: `node tools/replay-boxes.mjs` (defaults: 5 books, versions 1,2,3,4,5,6)
...
the v2/v3+ block line (`q<conf> ` → `q<conf>  `) in `src/boxes/encoder.ts` and
