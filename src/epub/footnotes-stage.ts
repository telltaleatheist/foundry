 ...
 *
 * The stage decodes a document, splices the decoded string, and re-encodes as
 ...
 * character in the book changed, on a run that reported success and removed the
 * markers it was asked to. So the decode is checked rather than assumed: a
 * replacement character means the bytes were not UTF-8, and a declared encoding
 * that says so means the publisher already told us.
 *
 ...
 * corrupted book.
 */
function requireUtf8(path: string, text: string): string {
  ...
  if (declared && !/^utf-?8$/i.test(declared)) {
    throw new Error(
      `footnotes: ${path} declares encoding "${declared}". This stage reads and writes UTF-8 only, `
      + `and transcoding a book on the way past is not something it will do quietly.`,
    );
  }
  if (text.includes('�')) {
    throw new Error(
      `footnotes: ${path} is not valid UTF-8 (a replacement character appears after decoding). `
...
export async function runEpubFootnotes(options: EpubFootnotesOptions): Promise<EpubFootnotesReport> {
  ...
  for (const doc of pkg.documents) {
    const source = requireUtf8(doc.path, entryText(byPath.get(doc.path)!));
    let units: ProseUnit[];
    try {
