...
# than a trap.
#
# The `nli-*` targets (torch + transformers + the DeBERTa weights, for the
# analysis ranker's entailment worker) were removed on 2026-09-25 with the
# worker: analysis ranks on a Crucible's decide door now (src/analyze/snap.ts),
# and the published v1 archives are orphaned on the release, not re-cast.
#
# OUTPUT (in <workdir>):
