...
**PAGES ON A BOX WITHOUT WSL — RULED, 2026-09-14.** PHASE15 §3.3/§3.5 first said
`pages` answers `enabled: false` in host mode (*"Windows is never a backend"*),
while Foundry's `page-reader.ts` reads pages on Windows today and no upstream
serves dots.ocr. Owen: *"i think that should go through crucible as well. at a
bare minimum, a crucible server will run on absolutely anything. it's an
extension of the foundry app. without it, foundry can simply open a file and look
...
the keys — is Crucible's, and Crucible decides whether the machine can bear it.
Deterministic work — rasterising, parsing, EPUB assembly, the bank and records,
the acronym lists — stays in Foundry and needs no "can this system do it" layer.
Foundry without an engine opens a book and reads it, nothing more. **Follow-on,
not this wave:** the analysis worker (a prebuilt Python running an NLI model on
the card) is inference by that rule and belongs in Crucible as its own class.
