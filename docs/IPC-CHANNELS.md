...
| `crucible:add-local` | Register the Crucible on this machine by reading its own `config.toml` — on Windows through `wsl.exe -d <distro> --exec`. The token is read and stored in main and never crosses this wire. The NAME argument may be empty and the doors card now sends it empty, which means the `[server] name` in that config.toml — the server naming itself, as on the other two roads (Owen, 2026-09-15: *"it should all be entered the exact same way"*). Refused while hosted. **Crucible `docs/PHASE15-HOST.md` §3.6 keeps this door until the Windows host ships and DELETES it then** — until there is a host, it is how a WSL server gets registered on Windows. |
