...
| `queue:enqueue-analysis` | Queue an analysis: the ranking (`analysis-rank`, a decide model) and the check behind it, both held on the GPU lane; answers `{ rank, analysis }`. Never routed, and REFUSED outright hosted — the host's queue does not know this request shape, and Foundry's own queue is invisible in a hosted window. |
