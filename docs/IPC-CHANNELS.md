...
| `host-ops:invoke` | Run one of the host's acts, named by id, from a node in the tree. |
| `host-ops:nodes` | One project's host-contributed nodes as they now stand — the first paint. |
| `host-ops:offers` | What the host registered at mount: its operations, without the doing. |
