...
interface Machine {
  ...
  ollama: OllamaFacts;
  held: Set<string>;
  /** The app's own kind, which package C renames. `'ollama'` is the local door. */
  ollamaIsTheServer: boolean;
  serverUrl: string;
}
...
function textGate(cls: ModelClass, machine: Machine): ActGate {
  ...
  if (!machine.ollamaIsTheServer) {
    return {
      lit: true,
      why: `This machine's language server is ${machine.serverUrl}, and the model is served there `
...
export async function actGates(): Promise<ActGates> {
  ...
  const machine: Machine = {
    ...
    ollamaIsTheServer: settings.llmServer === 'ollama',
    serverUrl: settings.llmServer === 'ollama' ? settings.ollamaUrl : settings.vllmUrl,
  };
