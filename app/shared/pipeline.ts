 ...
 * this pass at ~9 blocks/min against ~50 on the 9b-q8_0.
 *
 * ── THE MEASUREMENTS THAT USED TO LIVE IN A PICKER, KEPT HERE ───────────────
 *
 * Foundry offered three tags for this pass — 8-bit, 16-bit, 27B — in a
 * `cleanTextModelsFor(mlx)` list on two surfaces. That list is deleted with
 * every other model picker (Owen, 2026-09-15: *"we dont have any local models.
 * crucible handles all model orchestration"*), and two numbers it was built on
 * are worth more than the code:
 *
 *   * `qwen3.5:9b-bf16` is a GGUF, so ollama runs it on its llama.cpp path,
 *     and ollama pins that path to a single slot for this architecture ("model
 *     architecture does not currently support parallel requests",
 *     architecture=qwen35). The cleanup sends blocks in parallel, so one slot is
 *     the whole cost: **32 blocks/min measured on the M1 Ultra, against 61 for**
 *     **`qwen3.5:9b-mlx-bf16`** — the SAME weights at the SAME precision on
 *     ollama's MLX runner, which batches (measured 2026-09-08).
 *   * The 27B runs this pass at roughly a fifth of the 9B's rate (~9 against
 *     ~50 blocks/min, measured 2026-09-08).
 *
 * Whichever weights an ENGINE picks for the `clean` class, those ratios are what
 * a precision choice costs, and they are the reason the default is the 8-bit 9B
 * rather than the model every other act uses.
 */
